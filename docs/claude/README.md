...
- [payroll help desk](project_payroll_help_desk.md) — Slice 1 LIVE 2026-08-24: `payroll_tickets` queue (client-read-only, writes via workerSupportAssistant actions), inline Claude diagnosis (category/severity/EN+ES replies from readinessMirror+timesheets), worker `/c1/workers/payroll-help`, staff `/payroll-tickets` console; roadmap = Earnings v1 → approved actions → SMS/email intakes
