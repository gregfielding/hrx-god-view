...
const GigJobsBoardToggle: React.FC<GigJobsBoardToggleProps> = ({ jobOrder, onPostUpdated }) => {
  ...
  const buildPostData = (): CreatePostData => {
    return {
      ...
      showSkills,
      licensesCerts: ensureStringArray(licensesCerts),
      showLicensesCerts,
      experienceLevels: ensureStringArray(experienceLevels),
      showExperience,
      educationLevels: ensureStringArray(educationLevels),
      showEducation,
      languages: ensureStringArray(languages),
      showLanguages,
      physicalRequirements: ensureStringArray(physicalRequirements),
      showPhysicalRequirements,
      uniformRequirements: ensureStringArray(uniformRequirements),
      showUniformRequirements,
      requiredPpe: ensureStringArray(requiredPpe),
      showRequiredPpe,
      benefits: '',
  ...
  const saveRequirementFields = async () => {
    ...
    try {
      ...
      const updateData: any = {
        ...
        // Store physicalRequirements, uniformRequirements, and ppeRequirements as arrays (not strings)
        physicalRequirements: physicalRequirements.length > 0 ? physicalRequirements : undefined,
        uniformRequirements: uniformRequirements.length > 0 ? uniformRequirements : undefined,
