...
import { Box, Typography, Paper, TextField, IconButton, CircularProgress, Stack, Chip } from '@mui/material';
...
const AIAssistantChat: React.FC<AIAssistantChatProps> = ({ tenantId, userId, threadId, onThreadCreated }) => {
  ...
  return (
    <Paper variant="outlined" sx={{ display: 'flex', flexDirection: 'column', height: '100%', borderRadius: 0 }}>
      <Box ref={listRef} sx={{ flex: 1, overflowY: 'auto', p: 2 }}>
        {messages.length === 0 && (
          <Box sx={{ textAlign: 'center', mt: 10 }}>
            <PsychologyIcon color="primary" sx={{ fontSize: 48 }} />
            <Typography variant="h6" sx={{ mt: 1 }}>Ask anything about your CRM</Typography>
            <Typography variant="body2" color="text.secondary">Deals, contacts, tasks, emails, summaries, next steps…</Typography>
            <Box sx={{ mt: 2, display: 'flex', gap: 1, justifyContent: 'center', flexWrap: 'wrap' }}>
              {['What are my top stale deals?', 'Summarize my last 5 emails', 'Draft a follow-up for Acme'].map((t) => (
                <Chip key={t} label={t} variant="outlined" onClick={() => setInput(t)} />
              ))}
            </Box>
          </Box>
        )}
