 ...
 * Keep in sync with `functions/src/workerAiPrescreen/prescreenDynamicDedupe.ts`.
 *
...
/**
 * Reliable commute: concrete transport mode (not "not sure yet") and backup plan is Yes.
 */
...
/**
 * Walk dynamic steps in server plan order: apply synthetic answers for skipped steps so later
 * rules (e.g. commute after shift) see a consistent merged map.
 */
...
/** Explain-only: which steps would be skipped for QA / logging (no side effects on answers). */
export function explainPrescreenDynamicDedupePlan<
  T extends { id: string },
>(fullSteps: ReadonlyArray<T>, answers: PrescreenDedupeCoreAnswers, clientDynamic: Record<string, string>): {
  skipped: PrescreenDynamicDedupeSkip[];
  visibleIds: string[];
} {
  const { mergedDynamicAnswers, visibleSteps, skipped } = applyPrescreenDynamicDedupe(
    fullSteps,
    answers,
    clientDynamic,
  );
  void mergedDynamicAnswers;
  return {
    skipped,
    visibleIds: visibleSteps.map((s) => s.id),
  };
}
