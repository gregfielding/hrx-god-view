...
import type {
  JobOrderSnapshot,
  ResolvedPositionSnapshot,
} from '../../types/recruiter/jobOrder';
...
function isSnapshotted(snapshot: JobOrderSnapshot | null | undefined): boolean {
...
export function getEffectiveJobOrderField<T = unknown>(
  ...
  const snapValue = (joDoc.snapshot as unknown as Record<string, unknown>)[fieldKey];
...
export function getEffectiveJobOrderPositionField<T = unknown>(
  ...
  const match = positions.find(
    (p): p is ResolvedPositionSnapshot =>
      typeof p === 'object' &&
      p !== null &&
  ...
  const subValue = (match as unknown as Record<string, unknown>)[subField];
