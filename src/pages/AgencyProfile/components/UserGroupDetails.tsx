...
import { Box, Typography, TextField, Button, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Snackbar, Alert, Autocomplete, Tabs, Tab } from '@mui/material';
import { doc, getDoc, updateDoc, collection, getDocs, deleteDoc } from 'firebase/firestore';
import { db } from '../../../firebase';
import { useNavigate } from 'react-router-dom';
import AgencyProfileHeader from './AgencyProfileHeader';

const UserGroupDetails: React.FC<{ agencyId: string; groupId: string }> = ({ agencyId, groupId }) => {
  ...
  const fetchGroup = async () => {
    setLoading(true);
    try {
      const groupRef = doc(db, 'agencies', agencyId, 'userGroups', groupId);
  ...
  const handleEditSave = async () => {
    ...
    try {
      const groupRef = doc(db, 'agencies', agencyId, 'userGroups', groupId);
      await updateDoc(groupRef, { title: editForm.title, description: editForm.description, groupManagerIds });
      setSuccess(true);
      fetchGroup();
    } catch (err: any) {
  ...
  const handleTabChange = (_: React.SyntheticEvent, newIndex: number) => {
    ...
    const tabRoutes = [
      'overview', 'modules', 'locations', 'billing', 'contacts', 'customers', 'workforce', 'userGroups', 'jobOrders', 'shifts', 'timesheets', 'reports', 'aiSettings', 'activityLogs'
    ];
    if (newIndex !== 7) {
  ...
  const handleDeleteGroup = async () => {
    ...
    setError('');
    try {
      await deleteDoc(doc(db, 'agencies', agencyId, 'userGroups', groupId));
      navigate(`/agencies/${agencyId}?tab=6`);
    } catch (err: any) {
  ...
  return (
    <Box sx={{ p: 2, width: '100%' }}>
      {agency && (
        <>
          ...
          <Box display="flex" justifyContent="flex-end" mb={2}>
            <Button variant="outlined" onClick={() => navigate(`/agencies/${agencyId}?tab=6`)}>
      ...
      <Box display="flex" gap={2} mb={2}>
        <TextField label="Group Title" value={editForm.title} onChange={e => handleEditChange('title', e.target.value)} sx={{ flex: 1 }} />
        <TextField label="Description" value={editForm.description} onChange={e => handleEditChange('description', e.target.value)} sx={{ flex: 2 }} multiline minRows={2} />
        <Button variant="contained" onClick={handleEditSave} disabled={loading || !editForm.title || !editForm.description}>Save</Button>
      </Box>
      <Box mb={2}>
        <Autocomplete
          multiple
      ...
      <Box display="flex" gap={2} mb={2}>
        <Autocomplete
          ...
          sx={{ flex: 2 }}
        />
        <Button variant="contained" onClick={handleAddMember} disabled={!selectedWorker || loading}>Add</Button>
      </Box>
      ...
      <TableContainer component={Paper}>
        <Table size="small">
          ...
          <TableBody>
            {members.length === 0 ? (
              <TableRow><TableCell colSpan={5}>No members in this group.</TableCell></TableRow>
            ) : (
              members.map((member) => (
                <TableRow key={member.id}>
                  ...
                  <TableCell>
                    <Button size="small" variant="outlined" onClick={() => navigate(`/users/${member.id}`)}>View</Button>
                  </TableCell>
                  <TableCell>
                    <Button size="small" variant="outlined" color="error" onClick={() => handleRemoveMember(member.id)}>Remove</Button>
                  </TableCell>
                </TableRow>
              ))
            )}
      ...
      <Snackbar open={success} autoHideDuration={2000} onClose={() => setSuccess(false)}>
        <Alert severity="success" sx={{ width: '100%' }}>Group updated!</Alert>
      </Snackbar>
    </Box>
  );
};
