...
const LocationDetails: React.FC<LocationDetailsProps> = ({ agencyId, locationId, onBack }) => {
  ...
  const fetchLocation = async () => {
    if (!agencyId || !locationId) return;
    const locRef = doc(db, 'agencies', agencyId, 'locations', locationId);
  ...
  return (
    <Box sx={{ p: 2 }}>
      <Box display="flex" alignItems="center" justifyContent="space-between" mb={2}>
        ...
        <Button variant="outlined" onClick={onBack ? onBack : () => navigate(`/agencies/${agencyId}`)}>
