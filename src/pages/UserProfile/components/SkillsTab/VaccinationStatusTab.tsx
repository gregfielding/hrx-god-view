...
import { Box, Typography, TextField, Button, MenuItem, Grid, InputLabel, IconButton } from '@mui/material';
  ...
  return (
    <Box>
      ...
      {vaccinations.map((entry, idx) => (
        <Grid container spacing={2} key={idx} sx={{ mb: 2, border: '1px solid #333', borderRadius: 2, p: 2 }}>
          <Grid item xs={12} sm={4}>
            <TextField select label="Vaccine Type" fullWidth value={entry.type} onChange={e => handleFieldChange(idx, 'type', e.target.value)}>
              {vaccineTypes.map(type => <MenuItem key={type} value={type}>{type}</MenuItem>)}
            </TextField>
          </Grid>
          <Grid item xs={12} sm={4}>
            <TextField select label="Status" fullWidth value={entry.status} onChange={e => handleFieldChange(idx, 'status', e.target.value)}>
              {statusOptions.map(opt => <MenuItem key={opt} value={opt}>{opt}</MenuItem>)}
            </TextField>
          </Grid>
          <Grid item xs={12} sm={4}>
            <TextField label="Date of Last Dose" type="date" fullWidth value={entry.date} onChange={e => handleFieldChange(idx, 'date', e.target.value)} InputLabelProps={{ shrink: true }} />
          </Grid>
          ...
          <Grid item xs={12} sm={5}>
            <TextField label="Notes" fullWidth multiline minRows={2} value={entry.notes} onChange={e => handleFieldChange(idx, 'notes', e.target.value)} />
          </Grid>
      ...
      <Button variant="outlined" onClick={handleAdd}>Add Vaccination</Button>
      <Button variant="contained" color="primary" sx={{ ml: 2 }} onClick={handleSave}>Save Vaccinations</Button>
    </Box>
  );
};
