    ...
    if (bgStatusNeedsReview(st, res)) {
      out.push(
        makeActionItem({
          ...
          shortDescription: `${o.typeLabel || 'Screening'} — review result or status in Backgrounds.`,
          scope: { kind: 'global' },
          blocking: 'hard',
          sourceType: 'user_doc',
          sourceId: o.id,
      ...
      out.push(
        makeActionItem({
          ...
          shortDescription: `${o.typeLabel || 'Screening'} is still pending.`,
          scope: { kind: 'global' },
          blocking: 'soft',
          sourceType: 'user_doc',
          sourceId: o.id,
    ...
    if (required && (!hasFile || expired)) {
      out.push(
        makeActionItem({
          ...
          severity: 'high',
          actor: 'worker',
          title: `Certification required: ${name}`,
          shortDescription: expired ? 'Certification appears expired or invalid.' : 'Upload or renew this certification.',
          scope: { kind: 'global' },
          blocking: 'hard',
          sourceType: 'user_doc',
          sourceId: input.uid,
