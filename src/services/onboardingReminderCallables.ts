 ...
 *
 * Use case: the worker's been on Everee for a while (Everee shell exists)
 * but their `entity_employments` row is stuck with `payrollStatus: 'complete'`
 * from a prior payroll system (e.g. TempWorks pre-migration), which both
 *   (a) hides the Everee payroll step in the My Employment hub, and
 *   (b) tells `processWorkerOnboardingReminders` to skip them.
 *
 ...
 *   - `needs_sync`            — `evereeWorkerIds[evereeTenantId]` missing;
 *                               recruiter must click "Sync to Everee" first.
