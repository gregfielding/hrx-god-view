/**
 * Client + server–safe profile gates for `hiringLifecycle` dual-write on application create.
 * Mirrors `functions/src/workerAiPrescreen/evaluateAiPrescreenEligibility.ts` § eligibility checks
 * and maps missing primitives to persisted blocker codes.
 *
 * **CRA:** duplicate lives in `src/shared/profileEligibilityForHiringLifecycle.ts` (same logic).
 */

export type ProfileEligibilityForHiringLifecycleOptions = {
  /**
   * When false, resume and skills are not required.
   * Default true. Legacy `requireResumeOrWorkHistory` is honored if present.
   */
  requireResumeOrSkill?: boolean;
  /** @deprecated Honored if present for older tenant docs. */
...
/**
 * Returns whether the merged worker profile satisfies tenant AI prescreen eligibility flags
 * and which blocker codes to persist when it does not.
 */
