 ...
 *
 * **CRA client:** duplicate lives in `src/shared/hiringLifecyclePatch.ts` (imports `applicationStatus` from `src/utils`).
...
export type ApplyHiringLifecycleTimestampMetadataArgs = {
  /** Core fields from `buildHiringLifecyclePatch`. */
  core: HiringLifecycleCore;
  /** Previous persisted lifecycle (if any) for stage transition detection. */
  previous?: HiringLifecycleCore | null;
  /** ISO-8601 timestamp for `updatedAt` and new `stageEnteredAt` entry. */
  nowIso: string;
};
