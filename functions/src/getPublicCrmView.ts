...
export const getPublicCrmView = onCall({ cors: true }, async (request) => {
  try {
    ...
    const deals = rawDeals.map((deal) => {
      const primaryCompanyId = typeof deal.companyId === 'string'
        ? deal.companyId
        : ((deal.associations as { companies?: Array<string | { id?: string; isPrimary?: boolean }> } | undefined)?.companies || [])
            .map((c) => (typeof c === 'string' ? { id: c } : c))
            .find((c) => c?.isPrimary)?.id
          || ((deal.associations as { companies?: Array<string | { id?: string }> } | undefined)?.companies || [])
            .map((c) => (typeof c === 'string' ? c : c?.id))
            .find(Boolean)
          || null;
