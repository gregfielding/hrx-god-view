...
export const DEFAULT_MESSAGE_TYPES: MessageTypeConfig[] = [
  ...
  {
    id: 'worker_hired',
    label: 'Worker hired (welcome / offer)',
    category: 'onboarding',
    defaultChannels: ['sms', 'email', 'push'],
    critical: true,
    allowReply: true,
    ...
    aiAllowedToAutoSend: true,
    description:
      "Celebratory \"you are officially hired\" message sent when a worker onboarding pipeline is created for the first time for a worker + hiring entity. Trigger: worker_hired. Variables: {{firstName}}, {{hiringEntityName}}, {{hiringEntityId}}, {{entityKey}}, {{workerTypeLabel}} (localized), {{workerTypeLabelEn}}, {{workerTypeLabelEs}}, {{preferredLanguage}}. If no tenant automation rule is active, a default English or Spanish body is sent based on the worker's preferred language.",
    enabled: true,
  },
  ...
  },
  {
    ...
    label: 'Assignment Reminder (2h) — Address & Instructions',
    category: 'transactional',
    defaultChannels: ['sms', 'push'],
    critical: true,
    allowReply: true,
    ...
    aiAllowedToAutoSend: true,
    description:
      'Gig-worker variant of the T-2h reminder. Sends worksite address, parking, and site-entry instructions instead of the generic 2h reminder. Selected via tenant shiftReminderProfile=cort_gig.',
    enabled: true,
  },
  {
    id: 'assignment_reminder_15m_clockin',
    label: 'Assignment Reminder (15m) — Clock-in Link',
    category: 'transactional',
    defaultChannels: ['sms', 'push'],
    critical: true,
    allowReply: true,
    ...
    aiAllowedToAutoSend: true,
    description:
      'Gig-worker reminder sent 15 minutes before shift start with the Indeed Flex (or equivalent) clock-in URL. Selected via tenant shiftReminderProfile=cort_gig.',
    enabled: true,
  },
  {
    id: 'assignment_checkin_0h',
    label: 'Assignment Check-in (0h)',
    category: 'transactional',
    defaultChannels: ['sms', 'push'],
    critical: true,
    allowReply: true,
    ...
    aiAllowedToAutoSend: true,
    description:
      'Gig-worker check-in ping at scheduled start time. Confirms arrival and seeds the driver-meeting / walk-off prevention state machine. Selected via tenant shiftReminderProfile=cort_gig.',
    enabled: true,
  },
  {
    ...
    aiAllowedToAutoSend: true,
    description:
      'First escalation sent 23h before start if the worker has not replied YES or CANCEL to the T-24h reminder. Suppressed automatically at dispatch if cortConfirmation.state is already confirmed or cancelled.',
    enabled: true,
  },
  {
    id: 'assignment_reminder_22h_final',
    label: 'Assignment Reminder (22h) — Final',
    category: 'transactional',
    defaultChannels: ['sms', 'push'],
    critical: true,
    allowReply: true,
  ...
  {
    id: 'assignment_confirmation_receipt',
    label: 'Assignment Confirmation Receipt (reply)',
    category: 'transactional',
    defaultChannels: ['sms'],
    critical: false,
    allowReply: true,
    ...
    aiAllowedToAutoSend: true,
    description:
      'Short auto-reply sent when the cadence reply classifier detects a confirmation (YES / CONFIRMED). Closes the loop so the worker knows their reply was recognized.',
    enabled: true,
  },
  {
    id: 'assignment_cancellation_receipt',
    label: 'Assignment Cancellation Receipt (reply)',
    category: 'transactional',
    defaultChannels: ['sms'],
    critical: false,
    allowReply: true,
    ...
    aiAllowedToAutoSend: true,
    description:
      'Short auto-reply sent when the cadence reply classifier detects a cancellation (CANCEL / NO / CANT / SICK). Acknowledges the cancel and signals that the recruiter has been notified.',
    enabled: true,
  },
  {
    id: 'assignment_checked_in_receipt',
    label: 'Assignment Check-In Receipt (reply)',
    category: 'transactional',
    defaultChannels: ['sms'],
    critical: false,
    allowReply: true,
    ...
    aiAllowedToAutoSend: true,
    description:
      'Short auto-reply sent when the cadence reply classifier detects a check-in (HERE / ARRIVED / ON SITE). Closes the T+0 check-in loop and short-circuits the T+30 no-show alert.',
    enabled: true,
  },
  {
    id: 'assignment_walk_off_warning',
    label: 'Assignment Walk-Off Warning (reply)',
    category: 'transactional',
    defaultChannels: ['sms'],
    critical: true,
    allowReply: true,
    ...
    aiAllowedToAutoSend: true,
    description:
      'Sent back to a worker when the cadence reply classifier detects distress / walk-off risk ("no one is here", "I\'m leaving", "wrong place"). Explains that drivers and gig workers are paid from start time and should wait, while simultaneously alerting the recruiter team.',
    enabled: true,
  },
  {
    id: 'assignment_noshow_notify_recruiter',
    label: 'Assignment No-Show — Recruiter Alert',
    category: 'transactional',
    // No SMS/push to the worker — this type is the internal recruiter feed
    // alert generated 30 minutes after shift start if the worker has not
    // checked in.
    defaultChannels: [],
    critical: true,
    allowReply: false,
    ...
    aiAllowedToAutoSend: true,
    description:
      'Internal dashboard-feed notification written to each assigned recruiter when a worker has not confirmed arrival by T+30 minutes. Has no worker-facing side; purely a recruiter-side visibility event.',
    enabled: true,
  },
  {
