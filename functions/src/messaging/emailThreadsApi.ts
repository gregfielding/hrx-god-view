...
export const sendNewEmailApi = onRequest(
  ...
  async (request, response) => {
    ...
    try {
      ...
      // IMPORTANT: User-composed emails MUST send via Gmail API.
      // SendGrid is reserved for programmatic/system emails via the messaging framework.
      if (senderIdentity === 'sendgrid') {
        response.status(400).json({
          success: false,
          error: {
            code: 'SENDGRID_NOT_ALLOWED_FOR_COMPOSE',
            message: 'System sender is not allowed for user-composed emails. Please send via Gmail.',
          },
        });
        return;
      }
