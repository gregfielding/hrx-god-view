/**
 * QA report: prescreen decision integrity (contradictions, large deltas, gate mismatches).
 *
 * Usage (from repo root):
 *   npm run qa:prescreen-integrity -- --limit=80
 *   npm run qa:prescreen-integrity -- --dry-run --tenantId=TENANT --system-only --after=2026-01-01
 *
 * From `functions/`:
 *   npx ts-node src/scripts/qaPrescreenDecisionIntegrity.ts --limit=50
 */
...
async function main(): Promise<void> {
  ...
  while (rows.length < args.limit) {
    ...
    for (const userDoc of usersSnap.docs) {
      ...
      for (const d of intSnap.docs) {
        ...
        const ai = data.ai as Record<string, unknown> | undefined;
        const findings = analyzePrescreenInterviewIntegrity({ ai });
        ...
        rows.push({
          ...
          hiringDecision: hd?.decision,
          autoAdvance: hd?.eligibleForAutoAdvance,
          flags: ai?.flags,
          hardBlocks: ai?.hardBlocks,
          topOverrideReasons: Array.isArray(hd?.reasonCodes) ? hd?.reasonCodes : [],
          findings,
        });
      }
    }
  ...
  const payload = {
    ...
    dryRun: args.dryRun,
    args,
    stats: { usersScanned, interviewsSeen, findingsRows: rows.length },
    rows,
  };
