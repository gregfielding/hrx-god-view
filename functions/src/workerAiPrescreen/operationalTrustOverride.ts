 ...
 *
 * Used by `composePrescreenAiBundle` → `evaluateAiHiringDecision` (not a second scoring engine).
 */
...
/**
 * True when decline should be treated as **review** for hiring automation (minimum review floor).
 *
 * Does **not** override explicit high-severity compliance, physical mismatch, or missing experience
 * when the model already tagged `limited_relevant_experience` with a very weak experience band.
 */
export function computeOperationalTrustPromoteDeclineToReview(args: {
  ...
  /** Decline band is &lt;58 — only lift borderline declines, not deep F bands. */
